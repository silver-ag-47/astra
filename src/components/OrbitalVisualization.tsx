...
const Jupiter = ({ orbitRadius, isPaused, timeSpeed }: { orbitRadius: number; isPaused: boolean; timeSpeed: number }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const groupRef = useRef<THREE.Group>(null);
  const [angle, setAngle] = useState(Math.random() * Math.PI * 2);
  ...
  useFrame((state, delta) => {
    const speed = isPaused ? 0 : timeSpeed;
    if (meshRef.current) meshRef.current.rotation.y += 0.008 * speed; // Fast rotation
    if (groupRef.current) {
      setAngle(prev => prev + delta * 0.0084 * speed); // ~12 year orbit
      groupRef.current.position.x = Math.cos(angle) * orbitRadius;
      groupRef.current.position.z = Math.sin(angle) * orbitRadius;
    }
  });
      ...
      <group ref={groupRef}>
        <group rotation={[3.1 * Math.PI / 180, 0, 0]}>
          ...
          <Sphere args={[1.45, 32, 32]}>
            <meshBasicMaterial color="#f5e6d3" transparent opacity={0.15} />
          </Sphere>
        </group>
...
// Saturn with rings
const Saturn = ({ orbitRadius, isPaused, timeSpeed }: { orbitRadius: number; isPaused: boolean; timeSpeed: number }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const groupRef = useRef<THREE.Group>(null);
  ...
  useFrame((state, delta) => {
    const speed = isPaused ? 0 : timeSpeed;
    if (meshRef.current) meshRef.current.rotation.y += 0.007 * speed;
    if (groupRef.current) {
      setAngle(prev => prev + delta * 0.0034 * speed); // ~29 year orbit
      groupRef.current.position.x = Math.cos(angle) * orbitRadius;
      groupRef.current.position.z = Math.sin(angle) * orbitRadius;
    }
  });
  ...
  return (
    <>
      <Ring args={[orbitRadius - 0.06, orbitRadius + 0.06, 256]} rotation={[-Math.PI / 2, 0, 0]}>
        <meshBasicMaterial color="#f4d59e" transparent opacity={0.06} side={THREE.DoubleSide} />
      </Ring>
      <group ref={groupRef}>
        <group rotation={[26.7 * Math.PI / 180, 0, 0]}>
          ...
          <Sphere args={[1.12, 32, 32]}>
            <meshBasicMaterial color="#e8c878" transparent opacity={0.2} />
          </Sphere>
          {/* Ring system - multiple rings */}
          <Ring ref={ringsRef} args={[1.4, 1.7, 128]} rotation={[-Math.PI / 2, 0, 0]}>
            <meshBasicMaterial color="#c9b896" transparent opacity={0.7} side={THREE.DoubleSide} />
          </Ring>
          <Ring args={[1.72, 1.9, 128]} rotation={[-Math.PI / 2, 0, 0]}>
            <meshBasicMaterial color="#a89070" transparent opacity={0.5} side={THREE.DoubleSide} />
          </Ring>
          <Ring args={[1.95, 2.3, 128]} rotation={[-Math.PI / 2, 0, 0]}>
            <meshBasicMaterial color="#d4c4a8" transparent opacity={0.6} side={THREE.DoubleSide} />
          </Ring>
          <Ring args={[2.35, 2.5, 128]} rotation={[-Math.PI / 2, 0, 0]}>
            <meshBasicMaterial color="#b8a888" transparent opacity={0.3} side={THREE.DoubleSide} />
          </Ring>
        </group>
        <Html position={[0, -1.6, 0]} center>
          <span className="text-[8px] text-amber-200 font-mono">SATURN</span>
        </Html>
      </group>
    </>
  );
...
// Uranus with tilted axis and faint rings
const Uranus = ({ orbitRadius, isPaused, timeSpeed }: { orbitRadius: number; isPaused: boolean; timeSpeed: number }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const groupRef = useRef<THREE.Group>(null);
  const [angle, setAngle] = useState(Math.random() * Math.PI * 2);
  ...
  return (
    <>
      ...
      <group ref={groupRef}>
        {/* Uranus has extreme 97.8° tilt */}
        <group rotation={[97.8 * Math.PI / 180, 0, 0]}>
          <mesh ref={meshRef}>
          ...
          <Sphere args={[0.67, 32, 32]}>
            <meshBasicMaterial color="#a5f3fc" transparent opacity={0.25} />
          </Sphere>
          {/* Faint ring system */}
          <Ring args={[0.85, 0.95, 64]} rotation={[-Math.PI / 2, 0, 0]}>
            <meshBasicMaterial color="#94a3b8" transparent opacity={0.15} side={THREE.DoubleSide} />
          </Ring>
        </group>
...
// Neptune
const Neptune = ({ orbitRadius, isPaused, timeSpeed }: { orbitRadius: number; isPaused: boolean; timeSpeed: number }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const groupRef = useRef<THREE.Group>(null);
  const [angle, setAngle] = useState(Math.random() * Math.PI * 2);
  ...
  useFrame((state, delta) => {
    const speed = isPaused ? 0 : timeSpeed;
    if (meshRef.current) meshRef.current.rotation.y += 0.006 * speed;
    if (groupRef.current) {
    ...
    >
      <Canvas camera={{ position: [0, 20, 40], fov: 50 }} dpr={[1, 2]}>
        <Suspense fallback={<LoadingFallback />}>
          ...
          <Jupiter orbitRadius={earthOrbitRadius * 5.2} isPaused={isPaused} timeSpeed={timeSpeed} />
          <Saturn orbitRadius={earthOrbitRadius * 9.5} isPaused={isPaused} timeSpeed={timeSpeed} />
          <Uranus orbitRadius={earthOrbitRadius * 19.2} isPaused={isPaused} timeSpeed={timeSpeed} />
          <Neptune orbitRadius={earthOrbitRadius * 30} isPaused={isPaused} timeSpeed={timeSpeed} />
